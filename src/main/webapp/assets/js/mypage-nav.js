	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
			...
			if (json.length != 0) {
				...
				myPageBtns.innerHTML = "<input type='button' value='정보 수정하기' id='myInfoUpdateBtn' ><br>"
			...
			if (json.length != 0) {
				for (var i = 0; i < json.length; i++) {
					html += "<div>"
						+ "		<div> " + json[i].className + "</div> "
						+ "		<div> " + json[i].category + "</div>"
						+ "		<div> " + json[i].qnaTitle + "</div>"
						+ "		<div> " + json[i].qnaDate + "</div>"
						+ "		<div> " + json[i].userId + "</div>"
						+ "</div>";
